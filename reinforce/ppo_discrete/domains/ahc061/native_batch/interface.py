...
@runtime_checkable
class NativeBatchEnvProtocol(Protocol):
    @property
...
def ensure_native_batch_env(env: object) -> NativeBatchEnvProtocol:
    if isinstance(env, NativeBatchEnvProtocol):
        return cast(NativeBatchEnvProtocol, env)
    # Better error than AttributeError deeper in rollout loop.
    raise TypeError(f"env does not satisfy NativeBatchEnvProtocol: {type(env)!r}")
