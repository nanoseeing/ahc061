...
from ..domains.ahc061.native_batch import BatchEnv
...
def run_native_policy_episodes(
    *,
    env_id: str,
    episodes: int,
    seed: int,
    ...
    on_transition: Callable[[NativeTransition], None] | None = None,
    on_aux_transition: Callable[[NativeAuxTransition], None] | None = None,
    on_episode_end: Callable[[int, float, int], None] | None = None,
) -> NativeEpisodeStats:
    if str(env_id) != "AHC061Local-v0":
        raise ValueError(
            "native runner supports only env_id='AHC061Local-v0'; "
            f"got {env_id!r}"
        )
    policy_name = str(policy).strip().lower()
    if policy_name not in ("random", "model_stochastic", "model_greedy"):
        raise ValueError(f"unsupported native policy={policy!r}")
    ...
    if agent is not None:
        if tuple(agent.obs_shape) != tuple(obs_shape):
            raise ValueError(f"obs_shape mismatch: model={agent.obs_shape}, native_env={obs_shape}")
        if int(agent.action_dim) != int(a):
            raise ValueError(f"action_dim mismatch: model={agent.action_dim}, native_env={a}")
    was_training = bool(agent.training) if agent is not None else False
    if agent is not None:
        agent.eval()

    vecnorm: NativeVecNormalize | None = None
    if bool(vecnorm_enabled):
        vecnorm = NativeVecNormalize(
            num_envs=1,
            obs_shape=obs_shape,
        ...
        for epi in range(int(episodes)):
            ...
            for step in range(t_limit):
                ...
                try:
                    ...
                    raise RuntimeError(
                        "native step_observe_into failed (likely illegal action). "
                        "enable/use action mask for native rollout"
                    ) from exc
            ...
            # native env raises on illegal action; penalty component remains zero.
