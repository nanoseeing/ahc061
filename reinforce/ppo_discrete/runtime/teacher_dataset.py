...
def native_aux_to_teacher_opp_arrays(opp_param: Any, opp_valid: Any) -> tuple[np.ndarray, np.ndarray]:
    """Extract enemy-only slices (players 1..7) from native aux tensors."""
    ...
    if p.shape[0] < need + 1 or v.shape[0] < need + 1:
        raise ValueError(f"native aux player axis too short: opp_param={p.shape}, opp_valid={v.shape}")
