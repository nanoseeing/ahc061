...
def load_ext(*, verbose: bool = False):
    ...
    repo_root = mod_dir.parents[5]
    build_dir = repo_root / "reinforce" / "build" / "ahc061_batch_env" / f"torch_ext_build_pf{pf_particles}"
    ...
    _EXT = load(
        name=f"ahc061_batch_env_cpp_pf{pf_particles}_v1",
        sources=[str(src)],
