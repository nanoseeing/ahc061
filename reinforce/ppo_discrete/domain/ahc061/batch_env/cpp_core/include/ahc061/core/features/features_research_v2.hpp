...
#include "ahc061/core/features/feature_grid_utils.hpp"
...
inline void write_features_research_v2_from_common(const FeatureCommon& common, float* out_board) {
    ...
    for (int p = 0; p < M_MAX; p++) {
        fill_manhattan_dist_to_sources_clip(owner_source[static_cast<std::size_t>(p)], 18.0f, dist_plane.data());
        for (int idx = 0; idx < plane_size; idx++)
            out_board[(CH_R2_DIST_OWNER0 + p) * plane_size + idx] = dist_plane[static_cast<std::size_t>(idx)];

        fill_manhattan_dist_to_sources_clip(comp_source[static_cast<std::size_t>(p)], 18.0f, dist_plane.data());
        for (int idx = 0; idx < plane_size; idx++)
