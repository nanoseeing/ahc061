...
static constexpr int PAYLOAD_CODEC_HUFF91 = 1091;
static constexpr int PLAYER_HIDDEN_C = static_cast<int>(kModelMeta_player_hidden);
static constexpr int PPCONCAT_PRESET = static_cast<int>(kModelMeta_ppconcat_preset);
static constexpr int BLOCKPW_BASE_MODE = static_cast<int>(kModelMeta_blockpw_base_mode);
static constexpr std::uint32_t BLOCKPW_FRONT_I8_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_front_i8_mask);
static constexpr std::uint32_t BLOCKPW_BACK_I8_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_back_i8_mask);
static constexpr std::uint32_t BLOCKPW_PLAYER_I8_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_player_i8_mask);
static constexpr std::uint32_t BLOCKPW_FRONT_I4_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_front_i4_mask);
static constexpr std::uint32_t BLOCKPW_BACK_I4_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_back_i4_mask);
static constexpr std::uint32_t BLOCKPW_PLAYER_I4_MASK = static_cast<std::uint32_t>(kModelMeta_blockpw_player_i4_mask);
static constexpr int MERGE_MAIN_MODE = static_cast<int>(kModelMeta_merge_main_mode);
static constexpr int MERGE_PLAYER_MODE = static_cast<int>(kModelMeta_merge_player_mode);
static constexpr std::uint32_t FP8_AUX_MASK = static_cast<std::uint32_t>(kModelMeta_fp8_aux_mask);
static constexpr int MXDW_MAIN_MODE = static_cast<int>(kModelMeta_mxdw_main_mode);
static constexpr int MXDW_PLAYER_MODE = static_cast<int>(kModelMeta_mxdw_player_mode);
static constexpr int ARCH_VARIANT = static_cast<int>(kModelMeta_arch_variant);  // 0: ppconcat/full, 1: pcatonly
...
static_assert(
    PAYLOAD_CODEC == PAYLOAD_CODEC_BASE91 || PAYLOAD_CODEC == PAYLOAD_CODEC_BASE122 ||
        PAYLOAD_CODEC == PAYLOAD_CODEC_HUFF122 || PAYLOAD_CODEC == PAYLOAD_CODEC_HUFF91,
    "payload codec must be 91/122/1122/1091");
static_assert(PLAYER_HIDDEN_C > 0, "player_hidden must be positive");
static_assert(PPCONCAT_PRESET >= 0 && PPCONCAT_PRESET <= 13, "unsupported ppconcat preset");
static_assert(BLOCKPW_BASE_MODE == 0 || BLOCKPW_BASE_MODE == 1, "invalid blockpw base mode");
static_assert(MERGE_MAIN_MODE >= 0 && MERGE_MAIN_MODE <= 2, "invalid merge main mode");
static_assert(MERGE_PLAYER_MODE >= 0 && MERGE_PLAYER_MODE <= 2, "invalid merge player mode");
static_assert(MXDW_MAIN_MODE == 0 || MXDW_MAIN_MODE == 1, "invalid mxdw main mode");
static_assert(MXDW_PLAYER_MODE == 0 || MXDW_PLAYER_MODE == 1, "invalid mxdw player mode");
static_assert(ARCH_VARIANT == 0 || ARCH_VARIANT == 1, "invalid arch variant");
static_assert(FRONT_BLOCKS_N > 0, "front blocks must be positive");
static_assert(BACK_BLOCKS_N >= 0, "back blocks must be non-negative");
...
static std::vector<std::uint8_t> decode_payload(std::string_view s, std::size_t expected_size) {
    ...
    } else if constexpr (PAYLOAD_CODEC == PAYLOAD_CODEC_HUFF91) {
        const std::vector<std::uint8_t> packed = base91_decode(s);
        return huff122_decompress(packed, expected_size);
...
static float fp8e4m3fn_to_float(std::uint8_t v) {
    const int sign = (v & 0x80U) ? -1 : 1;
    const int exp = static_cast<int>((v >> 3) & 0x0FU);
    const int mant = static_cast<int>(v & 0x07U);
    if (exp == 0) {
        if (mant == 0)
            return 0.0F;
        const float x = std::ldexp(static_cast<float>(mant), -9);
        return sign > 0 ? x : -x;
    }
    if (exp == 15 && mant == 7) {
        return 0.0F;
    ...
    const float x = std::ldexp(frac, exp - 7);
    return sign > 0 ? x : -x;
}
...
static CompactModel load_compact_model() {
    ...
    const bool use_i8all = (PPCONCAT_PRESET == 1);
    const bool use_fp16 = (PPCONCAT_PRESET >= 2);
    const int blockpw_base_mode = BLOCKPW_BASE_MODE;  // 0: fp16 (+i8 mask), 1: int8 (+i4 mask)
    const int merge_main_mode = MERGE_MAIN_MODE;      // 0: fp16, 1: int8, 2: int4
    const int merge_player_mode = MERGE_PLAYER_MODE;  // 0: fp16, 1: int8, 2: int4
    ...
    const auto read_h = [&]() -> float { return h_vals[h_off++]; };
    ...
    const auto read_q_i8 = [&](std::vector<float>& dst, std::size_t out_c, std::size_t in_c) {
        read_q_i8_block(dst, 0, out_c, in_c);
    };

    const auto read_q_i8_rowmajor = [&](std::vector<float>& dst, std::size_t out_c, std::size_t in_c) {
        std::vector<float> scale(out_c);
        for (std::size_t oc = 0; oc < out_c; oc++) scale[oc] = read_h();
        for (std::size_t oc = 0; oc < out_c; oc++) {
            const float s = scale[oc];
            const std::size_t bo = oc * in_c;
            for (std::size_t ic = 0; ic < in_c; ic++) {
                const std::int8_t qv = static_cast<std::int8_t>(q_bytes[q_off++]);
                dst[bo + ic] = static_cast<float>(qv) * s;
            }
        }
    };

    const auto read_q_i8_rowmajor_block = [&](std::vector<float>& dst, std::size_t off, std::size_t out_c, std::size_t in_c) {
        std::vector<float> scale(out_c);
        for (std::size_t oc = 0; oc < out_c; oc++) scale[oc] = read_h();
        for (std::size_t oc = 0; oc < out_c; oc++) {
            const float s = scale[oc];
            const std::size_t bo = off + oc * in_c;
            for (std::size_t ic = 0; ic < in_c; ic++) {
                const std::int8_t qv = static_cast<std::int8_t>(q_bytes[q_off++]);
                dst[bo + ic] = static_cast<float>(qv) * s;
            }
    ...
    const auto read_q_i8_vec = [&](std::vector<float>& dst, std::size_t off, std::size_t n) {
        const float s = read_h();
        for (std::size_t i = 0; i < n; i++) {
            const std::int8_t qv = static_cast<std::int8_t>(q_bytes[q_off++]);
            dst[off + i] = static_cast<float>(qv) * s;
        }
    };

    const auto read_q_i4_block = [&](std::vector<float>& dst, std::size_t off, std::size_t out_c, std::size_t in_c) {
        std::vector<float> scale(out_c);
        for (std::size_t oc = 0; oc < out_c; oc++) scale[oc] = read_h();
        const std::size_t n_elem = out_c * in_c;
        const std::size_t need = (n_elem + 1ULL) / 2ULL;
        std::size_t elem_off = 0;
        for (std::size_t oc = 0; oc < out_c; oc++) {
            const float s = scale[oc];
            for (std::size_t ic = 0; ic < in_c; ic++) {
                const std::uint8_t packed = q_bytes[q_off + (elem_off >> 1)];
                const std::uint8_t nib = ((elem_off & 1ULL) == 0ULL) ? (packed & 0x0FU) : (packed >> 4);
                const int qv = (nib < 8U) ? static_cast<int>(nib) : (static_cast<int>(nib) - 16);
                dst[off + ic * out_c + oc] = static_cast<float>(qv) * s;
                elem_off++;
            }
        }
    ...
    const auto read_q_i4 = [&](std::vector<float>& dst, std::size_t out_c, std::size_t in_c) {
        read_q_i4_block(dst, 0, out_c, in_c);
    };

    const auto use_fp8_bit = [&](int bit) -> bool {
        return use_fp16 && (((FP8_AUX_MASK >> static_cast<std::uint32_t>(bit)) & 1U) != 0U);
    };

    const auto read_aux_scalar = [&](int fp8_bit) -> float {
        if (use_fp8_bit(fp8_bit)) {
            return fp8e4m3fn_to_float(q_bytes[q_off++]);
        }
        return read_h();
    };

    const auto read_aux_vec = [&](int fp8_bit, std::vector<float>& dst, std::size_t off, std::size_t n) {
        for (std::size_t i = 0; i < n; i++) dst[off + i] = read_aux_scalar(fp8_bit);
    };

    const auto read_dw_kernel = [&](int fp8_bit, std::vector<float>& dst, std::size_t ch, std::size_t block_idx) {
        const std::size_t bo = block_idx * ch;
        for (std::size_t ci = 0; ci < ch; ci++) {
            const std::size_t base = (bo + ci) * 9ULL;
            for (std::size_t k = 0; k < 9; k++) dst[base + k] = read_aux_scalar(fp8_bit);
        }
    };

    const auto read_dw_kernel_mxfp8 = [&](std::vector<float>& dst, std::size_t ch, std::size_t block_idx) {
        const std::size_t bo = block_idx * ch;
        for (std::size_t ci = 0; ci < ch; ci++) {
            const float s = read_h();
            const std::size_t base = (bo + ci) * 9ULL;
            for (std::size_t k = 0; k < 9; k++) {
                dst[base + k] = fp8e4m3fn_to_float(q_bytes[q_off++]) * s;
            }
        }
    };

    if constexpr (ARCH_VARIANT == 0) {
        if (use_fp16) {
            read_aux_vec(0, model.main_stem_w, 0, c * h);
            read_aux_vec(0, model.main_stem_gn_w, 0, h);
            read_aux_vec(0, model.main_stem_gn_b, 0, h);
        } else {
            read_q_i8(model.main_stem_w, h, c);
            read_q_i8_vec(model.main_stem_gn_w, 0, h);
            read_q_i8_vec(model.main_stem_gn_b, 0, h);
        }

        for (std::size_t bi = 0; bi < f; bi++) {
            const std::size_t bo = bi * h;
            const std::size_t bo_pw = bi * h * h;
            if (use_fp16 && MXDW_MAIN_MODE != 0) {
                read_dw_kernel_mxfp8(model.main_front_dw_w, h, bi);
            } else {
                read_dw_kernel(1, model.main_front_dw_w, h, bi);
            }
            if (use_fp16) {
                read_aux_vec(2, model.main_front_dw_gn_w, bo, h);
                read_aux_vec(2, model.main_front_dw_gn_b, bo, h);
                if (blockpw_base_mode == 0) {
                    if (((BLOCKPW_FRONT_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                        read_q_i4_block(model.main_front_pw_w, bo_pw, h, h);
                    } else if (((BLOCKPW_FRONT_I8_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                        read_q_i8_block(model.main_front_pw_w, bo_pw, h, h);
                    } else {
                        read_aux_vec(2, model.main_front_pw_w, bo_pw, h * h);
                    }
                } else if (((BLOCKPW_FRONT_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                    read_q_i4_block(model.main_front_pw_w, bo_pw, h, h);
                } else {
                    read_q_i8_block(model.main_front_pw_w, bo_pw, h, h);
                }
                read_aux_vec(2, model.main_front_pw_gn_w, bo, h);
                read_aux_vec(2, model.main_front_pw_gn_b, bo, h);
            } else {
                read_q_i8_vec(model.main_front_dw_gn_w, bo, h);
                read_q_i8_vec(model.main_front_dw_gn_b, bo, h);
                if (use_i8all) {
                    read_q_i8_block(model.main_front_pw_w, bo_pw, h, h);
                } else {
                    read_q_i4_block(model.main_front_pw_w, bo_pw, h, h);
                }
                read_q_i8_vec(model.main_front_pw_gn_w, bo, h);
                read_q_i8_vec(model.main_front_pw_gn_b, bo, h);
            }
        }
    }
    ...
    for (std::size_t bi = 0; bi < bk; bi++) {
        ...
        const std::size_t bo_pw = bi * h * h;
        if (use_fp16 && MXDW_MAIN_MODE != 0) {
            read_dw_kernel_mxfp8(model.main_back_dw_w, h, bi);
        } else {
            read_dw_kernel(1, model.main_back_dw_w, h, bi);
        }
        if (use_fp16) {
            read_aux_vec(2, model.main_back_dw_gn_w, bo, h);
            read_aux_vec(2, model.main_back_dw_gn_b, bo, h);
            if (blockpw_base_mode == 0) {
                if (((BLOCKPW_BACK_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                    read_q_i4_block(model.main_back_pw_w, bo_pw, h, h);
                } else if (((BLOCKPW_BACK_I8_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                    read_q_i8_block(model.main_back_pw_w, bo_pw, h, h);
                } else {
                    read_aux_vec(2, model.main_back_pw_w, bo_pw, h * h);
                }
            } else if (((BLOCKPW_BACK_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                read_q_i4_block(model.main_back_pw_w, bo_pw, h, h);
            } else {
                read_q_i8_block(model.main_back_pw_w, bo_pw, h, h);
            }
            read_aux_vec(2, model.main_back_pw_gn_w, bo, h);
            read_aux_vec(2, model.main_back_pw_gn_b, bo, h);
        } else {
            read_q_i8_vec(model.main_back_dw_gn_w, bo, h);
            read_q_i8_vec(model.main_back_dw_gn_b, bo, h);
            if (use_i8all) {
                read_q_i8_block(model.main_back_pw_w, bo_pw, h, h);
            } else {
                read_q_i4_block(model.main_back_pw_w, bo_pw, h, h);
            }
            read_q_i8_vec(model.main_back_pw_gn_w, bo, h);
            read_q_i8_vec(model.main_back_pw_gn_b, bo, h);
        }
    }

    if (use_fp16) {
        read_aux_vec(3, model.player_stem_common_w, 0, static_cast<std::size_t>(PLAYER_COMMON_INPUT_C) * ph);
        read_aux_vec(3, model.player_stem_enemy_w, 0, static_cast<std::size_t>(PLAYER_ENEMY_FEAT_INPUT_C) * ph);
        read_aux_vec(3, model.player_enemy_id, 0, static_cast<std::size_t>(PLAYER_BRANCH_N) * ph);
        read_aux_vec(3, model.player_stem_gn_w, 0, ph);
        read_aux_vec(3, model.player_stem_gn_b, 0, ph);
    } else {
        if (use_i8all) {
            read_q_i8(model.player_stem_common_w, ph, static_cast<std::size_t>(PLAYER_COMMON_INPUT_C));
            read_q_i8(model.player_stem_enemy_w, ph, static_cast<std::size_t>(PLAYER_ENEMY_FEAT_INPUT_C));
        } else {
            read_q_i4(model.player_stem_common_w, ph, static_cast<std::size_t>(PLAYER_COMMON_INPUT_C));
            read_q_i4(model.player_stem_enemy_w, ph, static_cast<std::size_t>(PLAYER_ENEMY_FEAT_INPUT_C));
        }
        read_q_i8_rowmajor(model.player_enemy_id, static_cast<std::size_t>(PLAYER_BRANCH_N), ph);
        read_q_i8_vec(model.player_stem_gn_w, 0, ph);
        read_q_i8_vec(model.player_stem_gn_b, 0, ph);
    }
    ...
    for (std::size_t bi = 0; bi < f; bi++) {
        ...
        if (use_fp16) {
            if (MXDW_PLAYER_MODE != 0) {
                read_dw_kernel_mxfp8(model.player_front_dw_w, ph, bi);
            } else {
                read_aux_vec(4, model.player_front_dw_w, bo_dw, ph * 9ULL);
            }
            read_aux_vec(5, model.player_front_dw_gn_w, bo, ph);
            read_aux_vec(5, model.player_front_dw_gn_b, bo, ph);
            if (blockpw_base_mode == 0) {
                if (((BLOCKPW_PLAYER_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                    read_q_i4_block(model.player_front_pw_w, bo_pw, ph, ph);
                } else if (((BLOCKPW_PLAYER_I8_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                    read_q_i8_block(model.player_front_pw_w, bo_pw, ph, ph);
                } else {
                    read_aux_vec(5, model.player_front_pw_w, bo_pw, ph * ph);
                }
            } else if (((BLOCKPW_PLAYER_I4_MASK >> static_cast<std::uint32_t>(bi)) & 1U) != 0U) {
                read_q_i4_block(model.player_front_pw_w, bo_pw, ph, ph);
            } else {
                read_q_i8_block(model.player_front_pw_w, bo_pw, ph, ph);
            }
            read_aux_vec(5, model.player_front_pw_gn_w, bo, ph);
            read_aux_vec(5, model.player_front_pw_gn_b, bo, ph);
        } else {
            read_q_i8_rowmajor_block(model.player_front_dw_w, bo_dw, ph, 9ULL);
            read_q_i8_vec(model.player_front_dw_gn_w, bo, ph);
            read_q_i8_vec(model.player_front_dw_gn_b, bo, ph);
            if (use_i8all) {
                read_q_i8_block(model.player_front_pw_w, bo_pw, ph, ph);
            } else {
                read_q_i4_block(model.player_front_pw_w, bo_pw, ph, ph);
            }
            read_q_i8_vec(model.player_front_pw_gn_w, bo, ph);
            read_q_i8_vec(model.player_front_pw_gn_b, bo, ph);
        }
    }
    ...
    if (use_fp16) {
        if constexpr (ARCH_VARIANT == 0) {
            if (merge_main_mode == 0) {
                read_aux_vec(6, model.merge_main_w, 0, h * h);
            } else if (merge_main_mode == 2) {
                read_q_i4(model.merge_main_w, h, h);
            } else {
                read_q_i8(model.merge_main_w, h, h);
            }
        }
        if (merge_player_mode == 0) {
            read_aux_vec(6, model.merge_player_w, 0, static_cast<std::size_t>(PLAYER_BRANCH_N) * ph * h);
        } else if (merge_player_mode == 2) {
            read_q_i4(model.merge_player_w, h, static_cast<std::size_t>(PLAYER_BRANCH_N) * ph);
        } else {
            read_q_i8(model.merge_player_w, h, static_cast<std::size_t>(PLAYER_BRANCH_N) * ph);
        }
        read_aux_vec(6, model.merge_gn_w, 0, h);
        read_aux_vec(6, model.merge_gn_b, 0, h);
        read_aux_vec(6, model.policy_w, 0, h);
        model.policy_b = read_aux_scalar(6);
    } else {
        if constexpr (ARCH_VARIANT == 0) {
            if (use_i8all) {
                read_q_i8(model.merge_main_w, h, h);
        ...
        read_q_i8_vec(model.merge_gn_w, 0, h);
        read_q_i8_vec(model.merge_gn_b, 0, h);
        read_q_i8_rowmajor(model.policy_w, 1ULL, h);
