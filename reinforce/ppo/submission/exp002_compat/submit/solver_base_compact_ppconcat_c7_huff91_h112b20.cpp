...
static constexpr int FEATURE_INPUT_C = 149;
...
static V<uint8_t> b91d(SV s) {
    ...
    for (unsigned char uc : s) {
        const int d = (uc <= 31) ? (static_cast<int>(uc) - 1 - (uc > 10) - (uc > 13))
                                 : (static_cast<int>(uc) - 3);
        if (v < 0) {
            v = d;
            continue;
        }
...
static V<uint8_t> hfd(const V<uint8_t>& blob, size_t expected_size) {
    ...
    for (int ln = 1; ln <= 15; ln++) {
        ...
        for (int sym = 0; sym < 256; sym++) {
            if (((blob[sym >> 1] >> ((sym & 1) << 2)) & 0x0F) != ln) {
                continue;
            }
            int node = 0;
            for (int k = ln - 1; k >= 0; k--) {
                const int bit = static_cast<int>((code >> k) & 1ULL);
    ...
    for (size_t i = 128ULL; i < blob.size(); i++) {
        ...
        for (int k = 7; k >= 0; k--) {
            const int bit = static_cast<int>((by >> k) & 1U);
...
static CM lcm() {
    ...
    CM model{};
    const size_t h = static_cast<size_t>(HIDDEN_C);
    const size_t ph = static_cast<size_t>(PLAYER_HIDDEN_C);
    const size_t f = static_cast<size_t>(10);
    const size_t bk = static_cast<size_t>(10);
    ...
    model.pscw.resize(static_cast<size_t>(35) * ph);
    model.psew.resize(static_cast<size_t>(16) * ph);
    model.peid.resize(static_cast<size_t>(7) * ph);
    ...
    model.mpw.resize(static_cast<size_t>(7) * ph * h);
    model.mgw.resize(h);
    model.mgb.resize(h);
    model.pw.resize(h);
    ...
    read_aux_vec(model.pscw, 0, static_cast<size_t>(35) * ph);
    read_aux_vec(model.psew, 0, static_cast<size_t>(16) * ph);
    read_aux_vec(model.peid, 0, static_cast<size_t>(7) * ph);
    ...
    read_q_i8_block(model.mpw, 0, h, static_cast<size_t>(7) * ph);
...
static void pla(const float* x, int channels, const float* w, float bias, float* ol) {
    ...
    const __m128 vbias4 = _mm_set1_ps(bias);
    int i = 0;
    for (; i < kVec; i += 8) {
        _mm256_storeu_ps(ol + i, vbias);
    }
    if (i + 4 <= CELL_MAX) {
        _mm_storeu_ps(ol + i, vbias4);
        i += 4;
    }
    for (; i < CELL_MAX; i++) {
        ol[i] = bias;
    ...
    for (int c = 0; c < channels; c++) {
        ...
        if (j + 4 <= CELL_MAX) {
            __m128 o = _mm_loadu_ps(ol + j);
            const __m128 s = _mm_loadu_ps(src + j);
            o = _mm_fmadd_ps(s, _mm_set1_ps(wc), o);
            _mm_storeu_ps(ol + j, o);
            j += 4;
        }
        for (; j < CELL_MAX; j++) {
            ol[j] += src[j] * wc;
    ...
    for (int g = 0; g < groups; g++) {
        ...
        for (int c = c0; c < c1; c++) {
            ...
            if (i + 4 <= CELL_MAX) {
                ...
                _mm_store_ps(t, v);
                sum_tail += t[0] + t[1] + t[2] + t[3];
                sq_tail += t[0] * t[0] + t[1] * t[1] + t[2] * t[2] + t[3] * t[3];
                i += 4;
            }
            for (; i < CELL_MAX; i++) {
                const float v = src[i];
        ...
        for (int c = c0; c < c1; c++) {
            ...
            if (apply_silu) {
                ...
                if (i + 4 <= CELL_MAX) {
                    __m256 v = _mm256_insertf128_ps(_mm256_setzero_ps(), _mm_loadu_ps(src + i), 0);
                    v = _mm256_mul_ps(_mm256_sub_ps(v, vmean), vstd);
                    v = _mm256_fmadd_ps(v, vgw, vgb);
                    v = sva(v);
                    _mm_storeu_ps(dst + i, _mm256_castps256_ps128(v));
                    i += 4;
                }
                for (; i < CELL_MAX; i++) {
                ...
                if (i + 4 <= CELL_MAX) {
                    ...
                    v = _mm_mul_ps(_mm_sub_ps(v, _mm256_castps256_ps128(vmean)), _mm256_castps256_ps128(vstd));
                    v = _mm_add_ps(_mm_mul_ps(v, _mm_set1_ps(gw)), _mm_set1_ps(gb));
...
static void asa(float* x, const float* z, size_t n) {
    ...
    if (i + 4 <= n) {
        __m128 v = _mm_add_ps(_mm_loadu_ps(x + i), _mm_loadu_ps(z + i));
        alignas(16) float t[4];
        _mm_store_ps(t, v);
        t[0] = silu(t[0]);
        t[1] = silu(t[1]);
        t[2] = silu(t[2]);
        t[3] = silu(t[3]);
        _mm_storeu_ps(x + i, _mm_load_ps(t));
        i += 4;
    }
    for (; i < n; i++) {
        x[i] = silu(x[i] + z[i]);
...
__attribute__((target("avx2,fma")))
static inline void addi(float* dst, const float* src, size_t n) {
    size_t i = 0;
    for (; i + 8 <= n; i += 8) {
...
static void rpl(
    ...
    A<float, 7 * PLAYER_HIDDEN_C * CELL_MAX> pcat;

    c1a(board.data(), IN_CHANNELS, HIDDEN_C, model.msw.data(), ym.data());
    gna(
        ym.data(),
        HIDDEN_C,
        8,
    ...
    for (int enemy = 0; enemy < 7; enemy++) {
        ...
        const int enemy_ch0 = 19 + (enemy + 1) * 16;
        ...
        for (size_t cidx = 0; cidx < ph; cidx++) {
            ...
            const __m128 vb4 = _mm_set1_ps(b);
            float* ydst = py + cidx * CELL_MAX;
            const float* csrc = cpj.data() + cidx * CELL_MAX;
            int i = 0;
            for (; i + 8 <= CELL_MAX; i += 8) {
                __m256 y = _mm256_loadu_ps(ydst + i);
                y = _mm256_add_ps(y, _mm256_loadu_ps(csrc + i));
                y = _mm256_add_ps(y, vb);
                _mm256_storeu_ps(ydst + i, y);
            }
            if (i + 4 <= CELL_MAX) {
                __m128 y = _mm_loadu_ps(ydst + i);
                y = _mm_add_ps(y, _mm_loadu_ps(csrc + i));
                y = _mm_add_ps(y, vb4);
                _mm_storeu_ps(ydst + i, y);
                i += 4;
            }
            for (; i < CELL_MAX; i++) {
                ydst[i] += csrc[i] + b;
    ...
    c1a(xm.data(), HIDDEN_C, HIDDEN_C, model.mmw.data(), ym.data());
    c1a(
        pcat.data(),
        7 * PLAYER_HIDDEN_C,
        HIDDEN_C,
    ...
    pla(xm.data(), HIDDEN_C, model.pw.data(), model.pb, ol.data());
}
...
static inline void efs(
    ...
    for (int idx = 0; idx < CELL_MAX; idx++) {
        am[idx] = reach[0][static_cast<size_t>(idx)] ? 1 : 0;
    }

    const A<int64_t, M_MAX>& sr = psr;
