...
from reinforce.ppo_discrete.cli.train_ppo import (
    _build_schedule_fns,
    _parse_schedule_expr,
    _schedule_progress,
...
class TestTrainPPOSchedule:
    def test_schedule_progress_bounds(self) -> None:
        assert _schedule_progress(1, 1) == 1.0
        assert _schedule_progress(1, 2) == 0.0
        assert _schedule_progress(2, 2) == 1.0
        assert _schedule_progress(3, 5) == pytest.approx(0.5, abs=1e-6)

    def test_parse_piecewise_expr_interpolates(self) -> None:
        fn, desc = _parse_schedule_expr(
    ...
    def test_build_schedule_fns_requires_clip_range_vf(self) -> None:
        cfg = PPOConfig(
            clip_range_vf=None,
            clip_range_vf_schedule_expr="linear(0.2,0.1)",
        )
        with pytest.raises(ValueError):
            _build_schedule_fns(cfg)
    ...
    def test_build_schedule_with_vf_clip_schedule(self) -> None:
        ...
        vf_fn, _vf_desc = schedules["vf_clip"]
